...
export default class BasicTask {
    ...
    list(task: string): TaskItemBtn[] {
        if (task === "build") {
            return [new TaskItemBtn("build-all", "编译"), new TaskItemBtn("build-html", "单独编译index.html")]
        } else if (task === "run") {
            ...
            return [new TaskItemBtn("others-config", "查看当前环境的配置"), new TaskItemBtn("others-log", "查看Output"), new TaskItemBtn("others-close", "尝试process.exit")]
        }
        return []
    }
